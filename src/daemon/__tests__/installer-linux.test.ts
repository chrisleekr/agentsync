...
describe("isRegisteredLinux", () => {
  ...
  test("returns false when systemctl is-enabled outputs 'not-found'", async () => {
    isEnabledStdout = "not-found\n";
    expect(await m.isRegisteredLinux()).toBe(false);
  });
});

// Additional coverage for startLinux when registered
describe("startLinux when registered", () => {
  test("calls systemctl --user start when service is registered", async () => {
    isEnabledStdout = "enabled\n";
    await m.startLinux();
    const startCall = execFileCalls.find(
      (c) => c.cmd === "systemctl" && c.args.includes("start") && c.args.includes("--user"),
    );
...
// Additional buildUnit structure checks
describe("buildUnit structure", () => {
  test("includes [Unit] section with Description", () => {
    const unit = m.buildUnit(["/usr/local/bin/agentsync"]);
    expect(unit).toContain("[Unit]");
    expect(unit).toContain("Description=");
  });

  test("includes [Service] section with Restart=on-failure", () => {
    const unit = m.buildUnit(["/usr/local/bin/agentsync"]);
    expect(unit).toContain("[Service]");
    expect(unit).toContain("Restart=on-failure");
  });

  test("includes [Install] section with WantedBy=default.target", () => {
    const unit = m.buildUnit(["/usr/local/bin/agentsync"]);
    expect(unit).toContain("[Install]");
    expect(unit).toContain("WantedBy=default.target");
  });

  test("appends 'daemon _run' to ExecStart for multi-element args", () => {
    const unit = m.buildUnit(["/usr/bin/bun", "/opt/app/cli.js"]);
    expect(unit).toContain("ExecStart=/usr/bin/bun /opt/app/cli.js daemon _run");
  });
});