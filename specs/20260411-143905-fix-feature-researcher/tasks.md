...
- [ ] T025 Execute `quickstart.md` Step 3 (happy-path workflow dispatch against the fix branch). Record the run ID, watch the run via `gh run watch`, and tick every acceptance checkbox in Step 3: (a) overall conclusion `success`; (b) no `MCP servers blocked by policy` line; (c) no `bun run check` in agent log; (d) `mcp__tavily__search` call visible with results; (e) new post-steps assertion step visible and exits `0`; (f) new issue created with correct title-prefix / labels / assignee OR noop issue; (g) if the issue is a gap-matrix (NOT noop), its body contains a markdown table with ≥1 row AND each row cites ≥1 source URL in its cells — closes `/speckit.analyze` second-pass F3 gap and proves SC-004 row-level quality on this run. Save the run ID for the PR description.

- [ ] T026 Execute `quickstart.md` Step 4 (SC-003 timing): record the `agent` job duration from T025 and note whether it is ≤ 8 minutes. Single-run excess is acceptable; note as a pending post-merge observation under SC-003.

- [ ] T027 Execute `quickstart.md` Step 5 (forced-failure walkthrough — FR-001 verification). **CRITICAL**: use a disposable `TAVILY_API_KEY` value like `tvly-INVALID`. Record the current valid value before overwriting. Dispatch the workflow. Confirm: (a) overall conclusion `failure` (red X); (b) agent job fails with the `::error::FR-001 violation` workflow annotation; (c) no new `[feature-research]` issue created. Then **restore** the original `TAVILY_API_KEY` value and re-run Step 3 to confirm the restore is correct. Save both run IDs (forced-failure + restore-verify) for the PR description.

- [ ] T028 Execute `quickstart.md` Step 6 (hash-guardrail regression test — FR-007). Hand-edit `.github/workflows/feature-research.lock.yml` by appending a trailing space or comment. Dispatch. Confirm the `activation` job fails with a hash-mismatch error. `git restore .github/workflows/feature-research.lock.yml` to revert. Re-dispatch and confirm green. Save run IDs.
...
- [ ] T033 Commit the full working tree as a single commit (or staged series if the maintainer prefers atomic commits per story). Files to include: `.github/workflows/feature-research.md`, `.github/workflows/feature-research.lock.yml`, `.github/aw/actions-lock.json`, `CLAUDE.md`, and all of `specs/20260411-143905-fix-feature-researcher/`. Use `docs(specs)` or `fix(workflow)` scope per repo convention. Do not commit until maintainer gives explicit authorization (per global CLAUDE.md).
